...
if [ "$1" == "" ]; then
    ...
    if [ "$1" == "1" ]; then    
       ...
       python run_experiment.py --molecule='H2' --random_seed='750' --basis_set='sto3g' --algorithm='VQE' \
       ...
       echo "HI"
       #MOL_PATH="$DATA_PATH/molecules/H2.txt"  
       python run_experiment.py --molecule='LiH'
       --random_seed=='750'
       --basis_set=='sto3g'
       --algorithm='ExactEigensolver'
       --transformation_type=='TransformationType.FULL'   ## FIXME
       --qubitmapping_type=='QubitMappingType.PARITY'     ## FIXME
       --two_qubit_reduce
       --vqe_optimizer=='SPSA' 
       #--vqe_optimizer=='COBYLA'
       #--vqe_optimizer=='L_BFGS_B'
       ...
       #--vqe_var_form=='RYRZ'
       --vqe_depth==3
       --vqe_entangler=='linear'
       ##--vqe_entangler='full'                           ## FIXME
       --num_shots==1000 
